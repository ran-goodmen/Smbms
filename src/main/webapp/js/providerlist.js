...
function deleteProvider(obj){
	$.ajax({
		...
		success:function(data){
			...
			}else if(data.num>0){//删除失败
				//alert("对不起，删除供应商【"+obj.attr("proname")+"】失败");
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.num+"】条订单，不能删除");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/selectOne?type=1&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/selectOne?type=2&proid="+ obj.attr("proid");
	});
