	...
	$.ajax({
		...
		success:function(data){
			if(data.success){//删除成功：移除删除行
				...
				window.location.href=path+"/listBill";
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/viewBill?type=1&billid="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/viewBill?type=2&billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
