...
<%@include file="/jsp/common/head.jsp"%>
        <div class="right">
            ...
            <div class="search">
           		<form method="get" action="${pageContext.request.contextPath }/listUser">
                   ...
                   <c:forEach var="user" items="${users.list }" varStatus="status">
					<tr>
						...
						<span><a class="deleteUser" href="javascript:;" userid=${user.id } username=${user.username }><img src="${pageContext.request.contextPath }/images/schu.png" alt="删除" title="删除"/></a></span>
							<!--$,{pageContext.request.contextPath}/deleteUserById?id=$,{user.id}-->
						</td>
					</tr>
				</c:forEach>
			</table>
			<input type="hidden" id="totalPageCount" value="${users.pages}"/>
		  	<c:import url="rollpage.jsp">
	          	<c:param name="totalCount" value="${users.total}"/>
	          	<c:param name="currentPageNo" value="${users.pageNum}"/>
	          	<c:param name="totalPageCount" value="${users.pages}"/>
          	</c:import>
        </div>
    </section>
